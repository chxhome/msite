...
gulp.task('iconfont', () => {
    return gulp.src('./src/fonts/' + fontName+'/*.svg')
        .pipe(iconfontCss({
            fontName: fontName,
            path: './src/fonts/_icontpl.mcss',//mcss模板文件
            targetPath: path.join(__dirname + '/src/mcss/' + fontName + '.mcss'),// '../../../src2/mcss/' + fontName + '.mcss',//The path where the (S)CSS file should be saved, relative to the path used in gulp.dest()//相对于formats文件所在目录
            //fontPath: '../fonts/',//未知用途
            cssClass: 'u-icon'
        }))
        .pipe(iconfont({
...
gulp.task('icondemo', function () {
 ...
 <html lang="zh-CN">
    ...
    <body>
        <div class="icons">`
    fs.readdirSync('./src/fonts/iconfont/').forEach(function (file) {
...
// sprites
let spriteBase = path.join(__dirname, '/src/sprite');//用于创建雪碧图的各个图片存放位置目录，每一个雪碧图对应一个存放多个图片的文件夹folder
let imgBase = path.join(__dirname, '/res/img');//创建的雪碧图存放位置，比如/res/sprite/sprite1创建一个/res/img/sprite1.png雪碧图
let cssBase = path.join(__dirname, '/src/mcss/sprite');//使用雪碧图的mcss代码存放位置
